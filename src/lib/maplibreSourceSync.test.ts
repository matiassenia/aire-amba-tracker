...
describe("syncMapLibreSources", () => {
  it("updates existing sources after map load", () => {
    const sources = new Map<string, { setData: ReturnType<typeof vi.fn> }>([
      ...
      [MAPLIBRE_SOURCE_IDS.historicalHeat, { setData: vi.fn() }],
      [MAPLIBRE_SOURCE_IDS.stations, { setData: vi.fn() }],
      [MAPLIBRE_SOURCE_IDS.groups, { setData: vi.fn() }],
      [MAPLIBRE_SOURCE_IDS.historicalGroups, { setData: vi.fn() }],
  ...
  it("passes the generated heat, group and hotspot feature counts to setData", () => {
    ...
    const data = {
      ...
      historicalHeat: { type: "FeatureCollection" as const, features: Array.from({ length: 3 }, (_, index) => point(index)) },
      ...
      historicalGroups: { type: "FeatureCollection" as const, features: [point(1)] },
    ...
    expect(sources.get(MAPLIBRE_SOURCE_IDS.historicalHeat)?.setData.mock.calls[0][0].features).toHaveLength(3);
    expect(sources.get(MAPLIBRE_SOURCE_IDS.groups)?.setData.mock.calls[0][0].features).toHaveLength(5);
    expect(sources.get(MAPLIBRE_SOURCE_IDS.historicalGroups)?.setData.mock.calls[0][0].features).toHaveLength(1);
